...
# --- 3. ALNS SOLVER CLASS (Upgraded with 2-Opt) ---
class AlnsSolver:
    ...
    def run(self, iterations):
        ...
        for i in range(iterations):
            ...
            # ⭐️ 1. Keep track of which routes were changed ⭐️
            modified_routes = self.greedy_insertion(removed_tasks, solution_to_modify=temp_solution)
            
            # ⭐️ 2. Apply 2-Opt to only the routes that were changed ⭐️
    ...
    def greedy_insertion(self, tasks_to_insert, solution_to_modify=None):
        # (Slight modification to return which routes were modified)
        sol = self.current_solution if solution_to_modify is None else solution_to_modify
        modified_routes = set() # Keep track of changed routes
    ...
    # ⭐️ 3. Implement the 2-Opt local search operator ⭐️
    def two_opt(self, solution, agv_id):
        """Improves a single route using the 2-Opt heuristic."""
        ...
        while improved:
            ...
            for i in range(1, len(route) - 2):
                for j in range(i + 1, len(route)):
                    ...
                    # Create a temporary new route with the swap
                    new_route = route[:i] + route[i:j][::-1] + route[j:]
                    
                    # Create temporary solutions to compare scores
                    ...
                    if new_score < original_score:
                        solution.routes[agv_id] = new_route # Keep the improvement
                        route = new_route
                        improved = True
                        break # Restart scan
                if improved:
...
if __name__ == '__main__':
    try:
        ...
        submission_filename = f"submission_shaw_2opt_{timestamp}.csv"
